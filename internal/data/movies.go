...
func (m MovieModel) Update(movie *Movie) error {
	//optimistic locking: a strategy where you read a record, take note of a version number (other methods to do this involve dates, timestamps or checksums/hashes) and check that the version hasn't changed before you write the record back.
...
func (m MovieModel) Delete(id int64) error {
	...
	//If rowsAffected is 0 that means that there was no record in DB to be changed, hence we throw a not found error
	if rowsAffected == 0 {
		return ErrRecordNotFound
	}
