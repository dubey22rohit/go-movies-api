...
func ValidatePasswordPlaintext(v *validator.Validator, password string) {
	...
	v.Check(len(password) <= 72, "password", "must be atmost 72 bytes long")
}
...
func ValidateUser(v *validator.Validator, user *User) {
	...
	//If the plaintext password in not nil, call the standalone
	//ValidatePasswordPlaintext() helper
	if user.Password.plaintext != nil {
		ValidatePasswordPlaintext(v, *user.Password.plaintext)
	}

	//If password hash is nil, it is because of mistake in the code
	//And not from client side, hence we panic in case that happens
